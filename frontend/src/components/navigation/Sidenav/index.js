...
function Sidenav({ color, brand, brandName, routes, ...rest }) {
  ...
  const collapseName = location.pathname
      .replace(/\/\d+/g, "")      // remove “/123” or “/1” etc.
      .replace(/^\/|\/$/g, "");
      ...
      >
        <RABox pt={3} pb={1} px={4} textAlign="center">
          ...
          <RABox component={NavLink} to="/" display="flex" alignItems="center">
            {brand && <RABox component="img" src={brand} alt="Brand" width="2rem" />}
            <RABox
                width={!brandName && "100%"}
                sx={(theme) => sidenavLogoLabel(theme, { miniSidenav })}
            >
              <RATypography component="h6" variant="button" fontWeight="medium" color={textColor}>
                {brandName}
              </RATypography>
            </RABox>
          </RABox>
