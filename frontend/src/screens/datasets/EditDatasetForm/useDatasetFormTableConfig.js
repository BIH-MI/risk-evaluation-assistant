 ...
 */
export function useDatasetFormTableConfig(tables, setTables, disabled = false) {
                ...
                prev.map(tbl =>
                    tbl.id === tblId
                        ? {
                            ...
                            attributes: [
                                ...tbl.attributes,
                                { id: `new-${Date.now()}`, name: '', dataType: 'STRING', isExcluded: false },
                            ],
                        }
                        : tbl
                )
    ...
    const tableStructureKey = tables.map(t => t.id).join(',');
    ...
    const columnsByTable = useMemo(() => {
        ...
        tables.forEach(tbl => {
            map[tbl.id] = [
                ...
                // FIX: Updated Delete Column with requested Icon style
                {
                    Header: 'Delete',
                    id: 'delete',
                    align: 'center',
